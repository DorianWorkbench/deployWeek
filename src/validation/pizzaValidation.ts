...
export class PizzaValidation {
  ...
  async validAddPizz(req: Request, res: Response, next: NextFunction) {
    if (!req.body.cost) {
      return res.status(400).json({ success: false, err: "cost-not-set" });
    } else if (!req.body.description) {
      return res
        .status(400)
        .json({ success: false, err: "description-not-set" });
    } else if (!req.body.pizzaName) {
      return res.status(400).json({ success: false, err: "pizzaName-not-set" });
    }
    ...
    if (status.status === 200) {
      return res
        .status(400)
        .json({ success: false, err: "pizza-already-exist" });
    }
