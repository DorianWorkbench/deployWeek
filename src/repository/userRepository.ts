...
export class UserRepository {
  ...
  async fetchUserById(dtoFetchUser: DTOfetchUser) {
    const result = await userScheme
      .findOne({ _id: dtoFetchUser.uuid })
  ...
  async fetchUserByEmail(dtoFetchUserByEmail: DTOfetchUserByEmail) {
    const result = await userScheme
      .findOne({ email: dtoFetchUserByEmail.email })
      .catch((err: mongoose.Error) => {
        return [{ status: 500 }, { success: false, err: "global-error" }];
      });
    if (!result) {
  ...
  async activeUser(dtoActiveUser: DTOactiveUser) {
    ...
    if (!result) {
      return [
        { status: 404 },
        { success: false, err: "user-relative-token-not-found" },
      ];
    }
