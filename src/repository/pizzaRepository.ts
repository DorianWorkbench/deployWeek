...
export class PizzaRepository {
  ...
  async insertPizza(dtoAddPizza: DTOaddPizza) {
    ...
    let result = await pizza.save().catch((err: mongoose.Error) => {
  ...
  async getPizza(dtoGetPizza: DTOgetOnePizza) {
    const result = await pizzaSchema
      .findById(dtoGetPizza.id)
  ...
  async getPizzaByName(dtoGetPizzaByName: DTOgetPizzaByName) {
    const result = await pizzaSchema
      .findOne({ pizzaName: dtoGetPizzaByName.pizzaName })
      .catch((err: mongoose.Error) => {
        return [{ status: 500 }, { success: false, err: "global-error" }];
      });
    if (!result) {
  ...
  async getPizzas() {
    let result = await pizzaSchema.find().catch((err: mongoose.Error) => {
      return [{ status: 500 }, { success: false, err: "global-error" }];
    });
  ...
  async deleteOnePizza(dtoDeletePizza: DTOdeleteOnePizza) {
    let result = await pizzaSchema
      .findOneAndDelete({ _id: dtoDeletePizza.id })
      .catch((err: mongoose.Error) => {
        return [{ status: 500 }, { success: false, err: "global-error" }];
      });
  ...
  async updatePizz(dtoUpdatePizza: DTOupdatePizza) {
    let result = await pizzaSchema
      .findOneAndUpdate({ _id: dtoUpdatePizza.id }, dtoUpdatePizza, {
        new: true,
